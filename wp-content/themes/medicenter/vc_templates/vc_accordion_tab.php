...
$output .= "\n\t\t\t" . '<li>';
        $output .= "\n\t\t\t\t" . '<div id="accordion-' . (empty($tab_id) ? sanitize_title($title) : esc_attr($tab_id)) . '"><h3 class="template-plus-2-after">' . $title . '</h3></div>';
		$output .= "\n\t\t\t\t" . '<div class="clearfix">';
        $output .= ($content=='' || $content==' ') ? __("Empty section. Edit page to add content here.", "medicenter") : "\n\t\t\t\t" . wpb_js_remove_wpautop($content);
        $output .= "\n\t\t\t\t" . '</div>';
        //$output .= "\n\t\t\t\t" . '</div></div>';
