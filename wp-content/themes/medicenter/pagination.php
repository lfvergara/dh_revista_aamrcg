<?php
function mc_pagination($ajax = false, $pages = '', $range = 2, $query_string = false, $outputEcho = true, $action = '', $top_margin = 'none')
{
	$showitems = ($range * 2)+1;  
	...
	if(1 != $pages)
	{
		...
		$output .= "</ul>";
		if($ajax)
			$output .= "<span class='mc-preloader pagination-preloader" . ($top_margin!="none" ? " " . esc_attr($top_margin) : "") . "'></span>";
		if($outputEcho)
			echo $output;
		else
			return $output;
